...
export function useFeatureState() {
  ...
  const createOrUpdate = useCallback(
      async (item: FeatureItem): Promise<FeatureItem> => {
        if (!item || typeof item !== 'object') {
          const error = new Error('Invalid feature item.')
        ...
        try {
          ...
          if (!saved || typeof saved !== 'object' || !saved.id) {
            throw new Error('Saved feature item is missing an id.')
          }
          setState((prev) => {
            const exists = prev.items.some((existing) => existing.id === saved.id)
            return {
              ...prev,
              items: exists ? prev.items.map((existing) => (existing.id === saved.id ? saved : existing)) : [saved, ...prev.items],
              loading: false,
              selectedId: saved.id,
            }
          })
          ...
          const message = error instanceof Error ? error.message : 'Failed to save feature item.'
          setState((prev) => ({ ...prev, loading: false, error: message }))
          throw error
        }
      },
