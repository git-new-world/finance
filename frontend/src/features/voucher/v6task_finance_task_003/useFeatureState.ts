  ...
  const createOrUpdate = useCallback(
      async (item: FeatureItem): Promise<boolean> => {
        ...
        try {
          ...
          setState((prev) => {
            const exists = prev.items.some((entry) => entry.id === saved.id)
            return {
              ...prev,
              items: exists
                ? prev.items.map((entry) => (entry.id === saved.id ? saved : entry))
                : [saved, ...prev.items],
              loading: false,
              selectedId: saved.id,
            }
          ...
          const message = error instanceof Error ? error.message : 'Failed to save voucher.'
          setState((prev) => ({ ...prev, loading: false, error: message }))
          return false
        }
      },
