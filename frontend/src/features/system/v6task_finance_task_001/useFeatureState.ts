...
export function useFeatureState() {
  ...
  const createOrUpdate = useCallback(
      async (data: Omit<FeatureItem, 'id'> & { id?: string | null }): Promise<FeatureItem> => {
        ...
        try {
          ...
          setState((prev) => {
            ...
            const items = exists
              ...
              : [saved, ...prev.items]
            return { ...prev, items, loading: false, selectedId: saved.id ?? null }
          })
          return saved
        } catch (error) {
